...
# =============================================================================
#   # TODO: spacy is installed and can be used, but if I run it here, my computer gets stuck...
#   # This thus still needs to be done on a computer on which spacy does run
#   # In def read_experiment_23(self, subject_ids, blocks) replace tokenized_sentence = nltk.word_tokenize(sentence) with:
#
#   tok_model = spacy.load('en_core_web_sm')
#   tok_sentence = tok_model(sentence)
#   tokenized_sentence = [token.text for token in tok_sentence]
#
# =============================================================================
...
class PereiraReader(FmriReader):
    ...
    def read_experiment_1(self, subject_ids, blocks):
        ...
        for subject_id in subject_ids:
            ...
            # the experiment has participants P01, and M01 until M17, skipping over M11 and M12
            # TODO: better to read out subject ids from folder
            if subject_id == 11:
                subject_id = "P01"
            if subject_id == 12:
                subject_id == 17
            ...
            # make blocks for sentences, pictures, or clouds
                ...
                if subject_id == "P01":
                    ...
                    datafile = scipy.io.loadmat(self.data_dir + "M{:0>2d}".format(subject_id) + "/data_180concepts_" + paradigms[paradigm_index] + ".mat")
            ...
            # make blocks for average
                ...
                for paradigm in range(paradigm_index):   
                    if subject_id == "P01":
                        ...
                        datafile = scipy.io.loadmat(self.data_dir + "M{:0>2d}".format(subject_id) + "/data_180concepts_" + paradigms[paradigm] + ".mat")
                    all_scans.append([])
                    for i in range(0, len(datafile["examples"])):
                        scan = datafile["examples"][i]
        ...
        for subject_id in subject_ids:
            ...
            if self.experiment == 2:
                if subject_id == "P01":
                    ...
                    datafile = scipy.io.loadmat(self.data_dir + "M{:0>2d}".format(int(subject_id)) + "/data_384sentences")
            else:
                if subject_id == "P01":
                        datafile = scipy.io.loadmat(self.data_dir + subject_id + "/data_243sentences")
                else:
                    datafile = scipy.io.loadmat(self.data_dir + "M{:0>2d}".format(int(subject_id)) + "/data_243sentences")
